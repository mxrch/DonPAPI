...
class MySeatBelt:
	...
	def GetEdgeSecrets(self):
		self.logging.info(f"[{self.options.target_ip}] {bcolors.OKBLUE}[+] Gathering Edge Secrets {bcolors.ENDC}")
		blacklist = ['.', '..']
		# Parse chrome
		# autres navigateurs ?
									...
									if guid != None:
										...
										if masterkey != None:
											if masterkey['status'] == 'decrypted':
												...
												self.logging.debug(
													f"[{self.options.target_ip}] {bcolors.WARNING}Error decrypting AES Key for Edge Local State  - Masterkey not decrypted{bcolors.ENDC}")
										else:
											self.logging.debug(
												f"[{self.options.target_ip}] {bcolors.WARNING}Error decrypting AES Key for Edge Local State with Masterkey- cant get masterkey {guid}{bcolors.ENDC}")
									else:
										self.logging.debug(
											f"[{self.options.target_ip}] {bcolors.WARNING}Error decrypting AES Key for Edge Local State with Masterkey - can t get the GUID of masterkey from blob file{bcolors.ENDC}")
								except Exception as ex:
									self.logging.debug(
